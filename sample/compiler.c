...
#define MAX_IDENTIFIER_NAME 255
extern int errno;
extern int annotate_flag;
 ...
 * -a : used from PS3 on to annotate each identifier with its symbol table entry id
 *
...
int main(int argc, char **argv) {
  ...
  char *stage, output_name[MAX_IDENTIFIER_NAME + 1];
  int opt;
  yyscan_t scanner;
  struct node *parse_tree;
  int error_count;

  strncpy(output_name, "output.s", MAX_IDENTIFIER_NAME + 1);
  stage = "mips";
  while (-1 != (opt = getopt(argc, argv, "o:s:a"))) {
    switch (opt) {
      case 'o':
        strncpy(output_name, optarg, MAX_IDENTIFIER_NAME);
        break;
      case 's':
        stage = optarg;
        break;
